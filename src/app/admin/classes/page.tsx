"use client";

import { useState, type SubmitEvent } from "react";
import Link from "next/link";
import { toast } from "sonner";
import { AlertCircle, ArrowLeft, BookOpen, Plus, ShieldAlert, Users } from "lucide-react";

import { Alert, AlertDescription } from "@/components/ui/alert";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { useRequireSession } from "@/hooks/use-session";
import { listAccounts } from "@/lib/auth";
import { createClass, schoolClasses, updateClass } from "@/lib/mock-data";

export default function AdminClassesPage() {
  const { account, status } = useRequireSession();
  const [classes, setClasses] = useState(() => [...schoolClasses]);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [className, setClassName] = useState("");
  const [dailyWordTarget, setDailyWordTarget] = useState("5");
  const [error, setError] = useState<string | null>(null);

  if (status !== "authenticated" || !account) {
    return (
      <div className="flex flex-1 items-center justify-center py-24 text-sm text-muted-foreground">
        Đang kiểm tra đăng nhập...
      </div>
    );
  }

  if (account.role !== "admin") {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 py-24 text-center">
        <ShieldAlert className="size-8 text-muted-foreground" />
        <p className="text-muted-foreground">Chỉ quản trị viên mới có quyền truy cập trang này.</p>
        <Link href="/dashboard" className="text-sm font-medium text-primary hover:underline">
          Về Dashboard
        </Link>
      </div>
    );
  }

  const studentCountByClass = (classId: string) =>
    listAccounts().filter((a) => a.account.role === "student" && a.account.classId === classId)
      .length;

  const resetForm = () => {
    setClassName("");
    setDailyWordTarget("5");
    setError(null);
  };

  const handleCreate = (event: SubmitEvent<HTMLFormElement>) => {
    event.preventDefault();
    setError(null);

    const target = Number(dailyWordTarget);

    if (!className.trim()) {
      setError("Vui lòng nhập tên lớp.");
      return;
    }
    if (!Number.isInteger(target) || target <= 0) {
      setError("Số từ mỗi ngày phải là số nguyên dương.");
      return;
    }

    createClass(className.trim(), target);
    setClasses([...schoolClasses]);
    setDialogOpen(false);
    resetForm();
    toast.success(`Đã tạo lớp ${className.trim()}.`);
  };

  const handleTargetChange = (id: string, value: string) => {
    const target = Number(value);
    if (!Number.isInteger(target) || target < 0) return;
    updateClass(id, { dailyWordTarget: target });
    setClasses([...schoolClasses]);
  };

  return (
    <div className="flex flex-1 flex-col bg-background">
      <header className="border-b border-border">
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between px-6 py-4">
          <Link
            href="/dashboard"
            className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground"
          >
            <ArrowLeft className="size-4" />
            Dashboard
          </Link>
          <div className="flex items-center gap-2">
            <div className="flex size-7 items-center justify-center rounded-lg bg-primary text-primary-foreground">
              <BookOpen className="size-3.5" />
            </div>
            <span className="font-heading text-base font-semibold">VocabApp</span>
          </div>
        </div>
      </header>

      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-6 py-10">
        <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
          <div className="flex flex-col gap-1">
            <h1 className="font-heading text-2xl font-semibold tracking-tight">Lớp học</h1>
            <p className="text-muted-foreground">
              Tạo lớp mới và chỉnh số từ vựng mục tiêu mỗi ngày cho từng lớp.
            </p>
          </div>

          <Dialog
            open={dialogOpen}
            onOpenChange={(open) => {
              setDialogOpen(open);
              if (!open) resetForm();
            }}
          >
            <DialogTrigger render={<Button />}>
              <Plus className="size-4" />
              Tạo lớp
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Tạo lớp mới</DialogTitle>
                <DialogDescription>Đặt tên lớp và số từ vựng mục tiêu mỗi ngày.</DialogDescription>
              </DialogHeader>

              <form onSubmit={handleCreate} noValidate className="flex flex-col gap-4">
                {error && (
                  <Alert variant="destructive">
                    <AlertCircle />
                    <AlertDescription>{error}</AlertDescription>
                  </Alert>
                )}

                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="className">Tên lớp</Label>
                  <Input
                    id="className"
                    value={className}
                    onChange={(e) => setClassName(e.target.value)}
                    placeholder="Lớp 10A2"
                  />
                </div>

                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="dailyWordTarget">Số từ mỗi ngày</Label>
                  <Input
                    id="dailyWordTarget"
                    type="number"
                    min={1}
                    value={dailyWordTarget}
                    onChange={(e) => setDailyWordTarget(e.target.value)}
                  />
                </div>

                <DialogFooter>
                  <Button type="submit">Tạo lớp</Button>
                </DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
        </div>

        <Card>
          <CardContent className="flex flex-col gap-1 py-4">
            {classes.map((cls, index) => (
              <div key={cls.id}>
                {index > 0 && <div className="my-3 h-px bg-border" />}
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="font-medium">{cls.className}</p>
                    <Badge variant="outline" className="mt-1 gap-1">
                      <Users className="size-3" />
                      {studentCountByClass(cls.id)} học sinh
                    </Badge>
                  </div>

                  <div className="flex items-center gap-2">
                    <Label htmlFor={`target-${cls.id}`} className="text-sm text-muted-foreground">
                      Số từ/ngày
                    </Label>
                    <Input
                      id={`target-${cls.id}`}
                      type="number"
                      min={0}
                      value={cls.dailyWordTarget}
                      onChange={(e) => handleTargetChange(cls.id, e.target.value)}
                      className="w-20"
                    />
                  </div>
                </div>
              </div>
            ))}
          </CardContent>
        </Card>
      </main>
    </div>
  );
}
